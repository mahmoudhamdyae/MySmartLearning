...
public data class PrivateChatFragmentArgs(
  ...
  public fun toSavedStateHandle(): SavedStateHandle {
    ...
    if (Parcelable::class.java.isAssignableFrom(User::class.java)) {
      result.set("teacher", this.teacher as Parcelable)
    } else if (Serializable::class.java.isAssignableFrom(User::class.java)) {
      result.set("teacher", this.teacher as Serializable)
    } else {
      throw UnsupportedOperationException(User::class.java.name +
          " must implement Parcelable or Serializable or must be an Enum.")
    }
  ...
  public companion object {
    ...
    public fun fromBundle(bundle: Bundle): PrivateChatFragmentArgs {
      bundle.setClassLoader(PrivateChatFragmentArgs::class.java.classLoader)
      val __course : Course?
      ...
      if (bundle.containsKey("teacher")) {
        if (Parcelable::class.java.isAssignableFrom(User::class.java) ||
            Serializable::class.java.isAssignableFrom(User::class.java)) {
          __teacher = bundle.get("teacher") as User?
        } else {
          throw UnsupportedOperationException(User::class.java.name +
              " must implement Parcelable or Serializable or must be an Enum.")
        }
        if (__teacher == null) {
          throw IllegalArgumentException("Argument \"teacher\" is marked as non-null but was passed a null value.")
        }
      } else {
        throw IllegalArgumentException("Required argument \"teacher\" is missing and does not have an android:defaultValue")
      }
      return PrivateChatFragmentArgs(__course, __user, __teacher)
    }

    @JvmStatic
    public fun fromSavedStateHandle(savedStateHandle: SavedStateHandle): PrivateChatFragmentArgs {
      val __course : Course?
      ...
      if (savedStateHandle.contains("teacher")) {
        if (Parcelable::class.java.isAssignableFrom(User::class.java) ||
            Serializable::class.java.isAssignableFrom(User::class.java)) {
          __teacher = savedStateHandle.get<User?>("teacher")
        } else {
          throw UnsupportedOperationException(User::class.java.name +
              " must implement Parcelable or Serializable or must be an Enum.")
        }
